...
import pytest
from src.utils import get_greeting, get_transactions, get_filtred_dict, get_cards_data, get_top_transactions, get_rate, \
    get_stock_prices
...
def test_get_transactions_valid():
    with patch("pandas.read_excel") as mock_read_excel:
        mock_read_excel.return_value = {'Дата': "2020-05-01 10:50:03",
                                        'Описание': 'Тинькофф', 'Кэшбэк': 1120}
        assert get_transactions(os.path.join(BASE_DIR, "data/operations.xlsx")) == {'Дата': "2020-05-01 10:50:03",
                                                                                    'Описание': 'Тинькофф',
                                                                                    'Кэшбэк': 1120}


def test_get_transactions_not_valid_path():
...
def test_get_greeting():
    with patch("src.utils.datetime") as mock_greeting:
        mock_greeting.datetime.now.return_value = datetime.datetime(year=2025, month=2, day=4, hour=5, minute=50,
                                                                    second=43)
        assert get_greeting() == "Доброй ночи"
        mock_greeting.datetime.now.return_value = datetime.datetime(year=2025, month=2, day=4, hour=7, minute=50,
                                                                    second=43)
        assert get_greeting() == "Доброе утро"
        mock_greeting.datetime.now.return_value = datetime.datetime(year=2025, month=2, day=4, hour=14, minute=56,
                                                                    second=43)
        assert get_greeting() == "Добрый день"
        mock_greeting.datetime.now.return_value = datetime.datetime(year=2025, month=2, day=4, hour=21, minute=50,
                                                                    second=43)
        assert get_greeting() == "Добрый вечер"


@patch("src.utils.get_transactions")
def test_get_filtred_dict_valid(mock_get_transactions, get_filtred_dict_valid):
...
@patch("src.utils.get_transactions")
def test_get_filtred_dict_not_date(mock_get_transactions, get_filtred_dict_valid):
    mock_get_transactions.return_value.to_dict.return_value = get_filtred_dict_valid
    assert get_filtred_dict("2025-02-01 21:50:03", os.path.join(BASE_DIR, "data/operations.xlsx")) == []


def test_get_filtred_dict_error():
    assert get_filtred_dict("2025/02/01 21:50:03", os.path.join(BASE_DIR, "data/operations.xlsx")) == [{}]
...
@pytest.mark.parametrize("date, adress, result",
...
@patch("src.utils.get_filtred_dict")
def test_get_top_transactions_valid(mock_get_top_transactions, get_top_transactions_valid,
                                    get_top_transactions_valid_result):
    mock_get_top_transactions.return_value = get_top_transactions_valid
    assert get_top_transactions("2025-02-05 21:50:03",
                                os.path.join(BASE_DIR, "data/operations.xlsx")) == get_top_transactions_valid_result


@patch("src.utils.get_filtred_dict")
def test_get_top_transactions_one(mock_get_top_transactions):
    mock_get_top_transactions.return_value = [
        {"Дата платежа": "05.02.2025 12:01:00", "Категория": "OK", "Описание": "Тинькофф", "Номер карты": "*9998",
         "Сумма платежа": -112}]
...
def test_get_stock_prices_valid(mock_requests_get_1, mock_json_load, currency_and_stock):
    ...
    mock_requests_get_1.return_value.json.side_effect = [
        {'success': True, 'base': 'usd', 'date': '2026-02-11', 'rates': {'rub': '77.505285388243220481'},
    ...
    assert get_stock_prices(os.path.join(BASE_DIR, "user_settings.json"), "USD", "RUB") == [
        {'stock': 'AAPL', 'price': 20192.8}, {'stock': 'AMZN', 'price': 15454.55}]
